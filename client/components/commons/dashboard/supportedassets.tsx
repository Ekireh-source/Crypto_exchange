...
export default function SupportedAssets() {
  ...
  return (
    <div className="w-full rounded-[8px] bg-[#13151a] overflow-hidden flex flex-col font-sans">
      {/* ── Crypto Section ── */}
        ...
        <div className="flex flex-col gap-4 mb-6">
          {loading ? (
            ...
            highlightedCrypto.map((asset) => {
              ...
              return (
                <div key={asset.id} className="flex items-center justify-between group cursor-pointer gap-2">
                  <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                    ...
                    <div className="flex flex-col min-w-0">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="text-[15px] sm:text-[16px] font-semibold text-white leading-tight truncate">
                          {asset.name}
                        </span>
                        <span className="text-[10px] sm:text-[11px] font-bold px-1.5 py-0.5 rounded bg-[#22252e] text-[#888c99] shrink-0">
                          {asset.network}
                        </span>
                      </div>
                      <span className="text-[13px] sm:text-[14px] text-[#888c99] truncate">
                        {available} {asset.symbol}
                      </span>
                    </div>
                  </div>
                  <button className="px-4 sm:px-5 py-2 rounded-full bg-[#22252e] text-white text-[14px] sm:text-[15px] font-semibold hover:bg-[#2a2d36] transition-colors shrink-0">
                    Buy
                  </button>
                </div>
              );
